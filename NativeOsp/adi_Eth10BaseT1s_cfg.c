 ...
 *                 FID = MAC 嶺뚮씭??嶺??꾩룆????(0x00~0x08)
 *                 Remote 0~7 = FID 0x01~0x08 (8??AD3304)
...
#define OSP_BRIDGE_DISABLE_ILAS 0

/* =========================================================================
   |                       INTERFACE REGMAPS (??ㅻ쾴??                        |
   ========================================================================= */
...
ADI_MEM_DATA_CRIT_CACHE
static ADI_E2BCORE_REG_ADDR_VAL aoIseledIf2IfRegMap[] =
{
    {
...
ADI_MEM_DATA_CRIT_CACHE
static ADI_E2BCORE_REG_ADDR_VAL aoIseledIf2FifoRegMap[] =
{
    {
        .nAddr = FIFO_CONTROL_NW_ADDR,
        .nVal = (FIFO_CONTROL_RESET_VAL & ~(BITM_FIFO_CONTROL_READ_MODE)) |
...
/* =========================================================================
   ...
   |   ?リ옇???AD3301 (SAIF2/SAIF3, SAIF4/SAIF5) ???깆젧?????                    |
   |   AD3304 IF4/IF5 ?????곕さ???곌떠???                                         |
   |     HOST_IF_TO_INTF_MUX2  : FIO0??얩댃E-IF0, FIO1??얩댃E-IF1              |
   |     HOST_IF_FROM_INT_MUX  : RxFIFO0??쒖¡O0, RxFIFO1??쒖¡O1             |
   |     IO_LV_TP_OUT_EN_SA_IF : SAIF4 TX enable (bit4)                   |
   |     IO_LV_TP_IP_EN_SA_IF  : SAIF5 RX enable (bit5)                   |
   ========================================================================= */

/* ???? ??ㅻ쾴??Remote RegMap 嶺뚮씞?뗥칰類욎뿉?(8???筌뤾퍓援??熬? ???됰뎄 HW ??뚮봽?? ???? */
...
{ \
    ...
    /* Keep FIO on interface 0; route ISELED on interface 1. */ \
    { \
        .nAddr = HOST_IF_TO_INTF_MUX2_NW_ADDR, \
        .nVal  = (HOST_IF_TO_INTF_MUX2_RESET_VAL & \
                  ~(BITM_HOST_IF_TO_INTF_MUX2_TO_INTF_MUX_10)) | \
    ...
    { \
        ...
        .nVal  = (HOST_IF_FROM_INT_MUX_RESET_VAL & \
                  ~(BITM_HOST_IF_FROM_INT_MUX_FROM_INTF_MUX_0 | \
                    BITM_HOST_IF_FROM_INT_MUX_FROM_INTF_MUX_1)) | \
                  ((0xaU << BITP_HOST_IF_FROM_INT_MUX_FROM_INTF_MUX_0) | \
                   (0x2U << BITP_HOST_IF_FROM_INT_MUX_FROM_INTF_MUX_1)) \
    }, \
    { \
        .nAddr = HOST_IF_INPUT_SELECTOR3_NW_ADDR, \
        .nVal  = (HOST_IF_INPUT_SELECTOR3_RESET_VAL & \
    ...
    /* ISELED routing copied from ADI ex_iseled: SAIF3=SIO_N, SAIF4=SIO_P. */ \
    { \
        .nAddr = HOST_IF_TO_INTF_MUX_NW_ADDR, \
        .nVal  = (HOST_IF_TO_INTF_MUX_RESET_VAL & \
                  ~(BITM_HOST_IF_TO_INTF_MUX_TO_INTF_MUX_2)) | \
                  ((0x1U << BITP_HOST_IF_TO_INTF_MUX_TO_INTF_MUX_2)) \
    }, \
    { \
    ...
    { \
        ...
        .nVal  = (HOST_IF_OUTPUT_SELECTOR1_RESET_VAL & \
                  ...
                  ((ENUM_HOST_IF_OUTPUT_SELECTOR1_IO4_OUTPUT_SELECTOR_ISELED0_SIOP << BITP_HOST_IF_OUTPUT_SELECTOR1_IO4_OUTPUT_SELECTOR)) \
    }, \
    { \
        .nAddr = HOST_IF_SA_IF_BUS_CONFIGURATION_NW_ADDR, \
        .nVal  = (HOST_IF_SA_IF_BUS_CONFIGURATION_RESET_VAL & \
...
/* 8??AD3304 Remote RegMap ??諛댁뎽 (FID = MAC 嶺뚮씭??嶺??꾩룆???? */
...
/* =========================================================================
   |               INTERFACE CONFIGURATIONS (??ㅻ쾴??2嶺??х몭?                   |
   ========================================================================= */
#if OSP_BRIDGE_DISABLE_ILAS
...
{ \
    ...
    [1U] = { \
        ...
        .nIfTopic  = 2, \
        .nNumIfReg = (sizeof(aoIseledIf2IfRegMap) / sizeof(ADI_E2BCORE_REG_ADDR_VAL)), \
        .aoIfRegData  = aoIseledIf2IfRegMap, \
        .nFifoTopic   = 6, \
        .nNumFifoReg  = (sizeof(aoIseledIf2FifoRegMap) / sizeof(ADI_E2BCORE_REG_ADDR_VAL)), \
        .aoFifoRegData = aoIseledIf2FifoRegMap, \
...
/* 8??Remote??IfConfig (?띠룄?ヨ????몃뎡 ?筌뤾쑬裕??怨룸츩 ?熬곣뫗?? */
...
/* =========================================================================
   |            REMOTE CONFIGURATIONS ??8??AD3304                         |
   ========================================================================= */
ADI_MEM_DATA_CRIT_CACHE
...
/* =========================================================================
   |  Eth10BaseT1sCfg_SetRemoteMac ??Remote 0 (嶺??뺢퀡????筌뤾퍓援? MAC ?띠룄???    |
   ========================================================================= */
...
/* =========================================================================
   |  Eth10BaseT1sCfg_SetRemoteMacByIdx ???筌뤾퍓????돠?Remote MAC ?띠룄???        |
   ========================================================================= */
void Eth10BaseT1sCfg_SetRemoteMacByIdx(uint32_t nIdx, const uint8_t* pMac6)
{
    /* ??Remote??RegMap ?꾩룄?ｈ굢?????????逾??*/
